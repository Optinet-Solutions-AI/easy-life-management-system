...
export default async function ShareholderDashboardPage() {
  ...
  const [expRes, revRes, foundRes, accountRes, budRevRes, budExpRes, guestRes, shRes] = await Promise.all([
    supabase.from('expenses').select('category, amount, currency, payment_date'),
